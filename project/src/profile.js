...
function getCharacterData() {
  ...
  let playerData = JSON.parse(
    utility.readJson("data/profiles/character_" + profileID + ".json")
  );
...
function getPurchasesData() {
        ...
        if (
          ...
          let itemCount =
            typeof data[invItems].upd != "undefined"
              ...
              : 1;
          let basePrice =
            items[templateId]._props.CreditsPrice >= 1
              ? items[templateId]._props.CreditsPrice
              : 1;
          data = addChildPrice(
            data,
  ...
  for (let invItems in data) {
    if (
      ...
      preparePrice =
        preparePrice > 0 && preparePrice != "NaN" ? preparePrice : 1;
      purchaseOutput +=
        '"' +
        data[invItems]._id +
        '":[[{"_tpl": "' +
...
function find(info, backendUrl) {
  ...
  return (
    ...
    '", "aid":' +
    ID +
    ', "lang":"de", "languages":{"en": "English","ru": "Русский","de": "Deutsch"}, "ndaFree":true, "queued":false, "taxonomy":341, "activeProfileId":"5c71b934354682353958e984", "backend":{"Trading":"' +
    backendUrl +
    '", "Messaging":"' +
    backendUrl +
    '", "Main":"' +
